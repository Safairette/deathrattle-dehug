...
public class Recipe_AdministerComaDrug : Recipe_Surgery
{
        ...
        RecipeDef recipe)
    {
        return MedicalRecipesUtility.GetFixedPartsToApplyOn(recipe, pawn, (Func<BodyPartRecord, bool>)(record =>
            pawn.health.hediffSet.GetNotMissingParts().Contains<BodyPartRecord>(record) &&
            !pawn.health.hediffSet.PartOrAnyAncestorHasDirectlyAddedParts(record) && !pawn.health.hediffSet.hediffs.Any(
                (Predicate<Hediff>)(x =>
                {
                    if (x.Part != record)
                        return false;
    ...
    public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
    {
        ...
        var ailment = HediffMaker.MakeHediff(HediffDefOfComatose.ArtificialComa, pawn) as Hediff_Comatose;
...
*/
    ...
    {
        if (pawn.Faction == billDoerFaction) return false;
        return true;
    }
}